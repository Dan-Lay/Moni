...
export function mapFinancialConfig(r: Record<string, any>): FinancialConfig {
  return {
    salarioLiquido: r.salario_liquido ?? 0,
    milhasAtuais: r.milhas_atuais ?? 0,
    metaDisney: r.meta_disney ?? 0,
    cotacaoDolar: r.cotacao_dolar ?? 5.0,
    reservaUSD: r.reserva_usd ?? 0,
    metaUSD: r.meta_usd ?? 0,
    cotacaoEuro: r.cotacao_euro ?? 5.65,
    reservaEUR: r.reserva_eur ?? 0,
    metaEUR: r.meta_eur ?? 0,
    cotacaoMediaDCA: r.cotacao_media_dca ?? 0,
    cotacaoMediaDCAEUR: r.cotacao_media_dca_eur ?? 0,
    ...
    limiteSeguranca: r.limite_seguranca ?? 0,
