...
import { Receipt, AlertTriangle, Globe } from "lucide-react";

const Expenses = () => {
  const { finance } = useFinance();
  const debits = finance.monthTransactions.filter((t) => t.amount < 0);
    ...
    <AppLayout>
      ...
      <div className="mt-6">
        <h2 className="mb-3 flex items-center gap-2 text-sm font-semibold text-muted-foreground">
          <Receipt className="h-4 w-4" /> Últimas Transações ({debits.length})
        </h2>
        {debits.length === 0 ? (
            ...
            {debits.slice(0, 20).map((t, i) => (
              <motion.div key={t.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                ...
                <div className="text-right ml-3">
                  ...
                  {t.iofAmount > 0 && (
                    <p className="text-[10px] text-muted-foreground font-mono">IOF: R$ {t.iofAmount.toFixed(2)}</p>
                  )}
                  {t.milesGenerated > 0 && (
